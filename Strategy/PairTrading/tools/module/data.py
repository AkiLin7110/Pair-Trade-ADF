...
def get_tidyData(symbol='BTCUSDT', data_type='ufutures', start = '2022-01-01', end = '2023-01-01'):
    ...
    # ticker_path = r'I:\我的雲端硬碟\PAIR_TRADE_BICENTIVE\Trading-Universe-main\raw_klines\{symbol}_{data_type}\*.zip"'
    ticker_path = sorted(ticker_path)
    if start != None and end!= None:
        ticker_df = pd.DataFrame(ticker_path)
        ticker_df.index = pd.to_datetime(ticker_df[0].apply(lambda x:x[-14:-4]), format = '%Y_%m_%d')
        ticker_df = ticker_df[start :end ]
        ticker_path = ticker_df[0].to_list()
