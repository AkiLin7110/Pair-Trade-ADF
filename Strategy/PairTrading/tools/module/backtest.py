...
def backtestingPair(input_arr, fund=100, leverage=0, takerFee=0.0004, 
    ...
    if exit_lossOut == True and (lossOut_condition == 1 or lossOut_condition == 2):
        lossOut_specialCase = False #原本是True
    else:
    ...
    for i in range(len(openA_arr)):
        ...
        # long position  ##
            ...
            if (stopProfit==False and stopLoss==False) or (stopLoss==True and lossOut_specialCase==False):
                ...
                # closeA_arr[i] / openA_arr[exT]
                profit = orderSizeA * (openA_arr[i+1] - openA_arr[i]) + orderSizeB * (openB_arr[i] - openB_arr[i+1])
                # profit = orderSizeA * (closeA_arr[i] / openA_arr[exT]) + orderSizeB * (openB_arr[exT] -  closeB_arr[i] )
            ...
            if exitShort_arr[i] == True or i == len(openA_arr)-2 or stopLoss or stopProfit or stopTime: 
                if (stopProfit==False and stopLoss==False) or (stopLoss==True and lossOut_specialCase==False):
                    pl_round = orderSizeA * (closeA_arr[i] / openA_arr[exT]) + orderSizeB * (openB_arr[exT] -  closeB_arr[i] )
                    # pl_round = orderSizeA * (openA_arr[i+1] - openA_arr[t]) + orderSizeB * (openB_arr[t] - openB_arr[i+1])
                    profit_fee = pl_round - orderMoney*feeRate - (orderMoney+pl_round)*feeRate
                else:
                    # pl_round = orderSizeA * (exitPriceA - openA_arr[t]) + orderSizeB * (openB_arr[t] - exitPriceB)
                    pl_round = orderSizeA * (closeA_arr[i] / openA_arr[exT]) + orderSizeB * (openB_arr[exT] -  closeB_arr[i] )
                    if stopLoss and lossOut_specialCase:
                        profit_fee = pl_round - orderMoney*feeRate - (orderMoney+pl_round)*stopLoss_feeRate
                    else:
                        profit_fee = pl_round - orderMoney*feeRate - (orderMoney+pl_round)*feeRate
        ...
        # short position
            ...
            if exitBuyToCover_arr[i] == True or i == len(openA_arr)-2 or stopLoss or stopProfit or stopTime: 
                ...
                profit_list_realized.append(profit_realized)
                profit_fee_list_realized.append(profit_fee_realized)
                profit_fee_list_realized_time.append(i)
...
def dictType_output(output_tuple):
    ...
    output_dict = {'buy': output_tuple[0], 'sell': output_tuple[1], 'sellshort': output_tuple[2], 'buytocover': output_tuple[3],
                   ...
                   'order_value':  output_tuple[9], 'order_type':  output_tuple[10]}
...
# exit_timeOut=True
# exParam1 = max_trade #代表最多可以跑幾期
# from_exit_condition=False
...
# @jit(nopython=True)
def backtestingPair2(input_arr, fund=100, leverage=0, takerFee=0.0004, 
                    slippage=0.0001, stopLoss_slippageAdd=0.001,
                    exit_timeOut=False, exParam1=20, from_exit_condition=False,
                    exit_profitOut=False, exParam2=0.02,
                    exit_lossOut=False, lossOut_condition=1, exParam3=0.05, risk_control=False, rc_percent=0.5,
    ...
    if exit_lossOut == True and (lossOut_condition == 1 or lossOut_condition == 2):
        lossOut_specialCase = True #原本是True
    else:
        lossOut_specialCase = False
    ...
    for i in range(len(openA_arr)):
        ...
        if exit_profitOut == True:       
            if exitSwitch == True:
                if LS == 'L':
                    stopProfit = (orderSizeA * (closeA_arr[i] - openA_arr[exT] ) + orderSizeB * ( openB_arr[exT] - closeB_arr[i]))/fund_money >= exParam2

                elif LS == 'S':
                    stopProfit = (orderSizeA * (openA_arr[exT] - closeA_arr[i]) + orderSizeB * (closeB_arr[i] - openB_arr[exT] ))/fund_money >= exParam2
        ...
        if exit_lossOut == True:       
            if exitSwitch == True:
                ...
                if lossOut_condition == 1:
                    if LS == 'L':
                        stopLoss =  (orderSizeA * (closeA_arr[i] - openA_arr[exT] ) + orderSizeB * ( openB_arr[exT] - closeB_arr[i]))/fund_money <= -exParam3

                    elif LS == 'S':
                        stopLoss =   (orderSizeA * (openA_arr[exT] - closeA_arr[i]) + orderSizeB * (closeB_arr[i] - openB_arr[exT] ))/fund_money  <= -exParam3
        ...
        if LS == '':
            ...
            if entryLong_arr[i] == True:
                if A_beta == None:
                    orderSizeA = (fund_money / 2) / openA_arr[i+1]
                    orderSizeB = (fund_money / 2) / openB_arr[i+1]
                    order_value.append([ (fund_money / 2) ,  (fund_money / 2) ])
                else:
                    k = sympy.symbols("k")
                    ans = sympy.solve([openB_arr[i+1]*k +  A_beta*openA_arr[i+1]*k  - fund_money] , [k])
                    k = float(ans[k])
                    orderSizeA = k*A_beta
                    orderSizeB = k
                    order_value.append([A_beta*openA_arr[i+1]*k , openB_arr[i+1]*k])
        ...
        # long position  
            ...
            pl_round = orderSizeA * (openA_arr[i+1] - openA_arr[exT]) + orderSizeB * ( openB_arr[exT] - openB_arr[i+1] )
            if stopLoss and lossOut_specialCase:
                profit_fee = profit - orderMoney*feeRate - (orderMoney + profit)*stopLoss_feeRate
            else:
                profit_fee = profit - orderMoney*feeRate - (orderMoney + profit)*feeRate
            ts += 1
            profit_list.append(profit)
            profit_fee_list.append(profit_fee)
            ...
            #紀錄關艙瞬間的總 profit # Realized PnL
            if flag == True: 
                profit_realized  = pl_round
                if stopLoss == True and lossOut_specialCase == True: #只有停損出場是特例
                    profit_fee_realized = pl_round - orderMoney*feeRate - (orderMoney + pl_round)*stopLoss_feeRate
                else:
                    profit_fee_realized = pl_round - orderMoney*feeRate - (orderMoney + pl_round)*feeRate
        ...
        # short position
            ...
            pl_round = orderSizeA * (openA_arr[exT] - openA_arr[i+1]) + orderSizeB * ( openB_arr[i+1] - openB_arr[exT])
            if stopLoss and lossOut_specialCase:
                profit_fee = profit - orderMoney*feeRate - (orderMoney + profit)*stopLoss_feeRate
            else:
                profit_fee = profit - orderMoney*feeRate - (orderMoney + profit)*feeRate
    ...
    return buy, sell, sellshort, buytocover, profit_list, profit_fee_list, profit_list_realized, profit_fee_list_realized, profit_fee_list_realized_time, order_value, order_type
