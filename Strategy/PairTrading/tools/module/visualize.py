...
class Performance:
    ...
    def __init__(self, df_price, output_dict, time_index, fund=100, Name='Strategy'):
        ...
        self.percent_return_series = self.return_series - self.return_series.shift(1)  # 扣手續費後的淨獲利
    ...
    def calculate_result(self):
        ...
        self.L_tradePeriod = [self.output_dict['sell'][num]-self.output_dict['buy'][num] for num in range(len(self.output_dict['sell']))]  #buy進場位置 - sell出場位置
        self.S_tradePeriod = [self.output_dict['buytocover'][num]-self.output_dict['sellshort'][num] for num in range(len(self.output_dict['buytocover']))] #sellshort進場位置 - buytocover出場位置
    ...
    def show_performance(self, days_of_year=365):
        ...
        if len(self.profit_fee_list_realized) != 0:
            ...
            winRate = len([i for i in self.profit_fee_list_realized[2:] if i > 0])/ len(self.tradePeriod)
            winRate = np.round(winRate*100,2)
        else:
            winRate = np.nan
        ...
        print(f'Start time : { self.time_index[0] }') 
        print(f'End time : { self.time_index[-1] }') 
        print(f'Cumulative Return: {np.round(performance_index_dict["Cumulative Return"],2)}%') # self.percent_return_series.cumsum()
